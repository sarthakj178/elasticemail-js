...
var sendMailUsingTemplate = function(
    ...
    var ccEmailAddresses =
        ccEmailAddresses.length > 0 ? ccEmailAddresses.join(SEPARATOR) : "";
    var bccEmailAddresses =
        bccEmailAddresses.length > 0 ? bccEmailAddresses.join(SEPARATOR) : "";
    ...
    Object.keys(emailParams).forEach(key => {
        formData[ADDITIONAL_PARAM_PREFIX + key] = emailParams[key]
            ? emailParams[key]
            : "<b></b>";
    });
    ...
    if (attachmentsS3Paths.length > 0) {
        attachmentsS3Paths.forEach(path => {
            ...
            getAttachmentPromises.push(
                s3Accessor.getFile(s3Bucket, s3Key).then(data => {
    ...
    formData[[ATTACHMENT_KEY]] = attachmentNames.join(SEPARATOR);
    return Promise.all(getAttachmentPromises).then(
        responses => {
            return request({
                method: "POST",
                url: SEND_EMAIL_URL,
                ...
                response => {
                    try {
                        ...
                        console.error(
                            "Invalid response from ElasticEmail",
                            error
                        );
                        return Promise.reject(
                            "Invalid response from ElasticEmail"
